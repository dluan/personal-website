...
export function Label({ indicator, children = null }: PropsWithChildren<LabelProps>) {
  const percentage = Math.min(100, Math.max(0, indicator))

  return (
    <span className={clsx('label')}>
      {children}
      <span className={clsx('label__indicator')} style={{ width: `${percentage}%` }} />
    </span>
  )
}
...
function Labels({ title, children }: LabelsProps) {
  return (
    <div className={clsx('flex flex-col items-start gap-2', 'sm:flex-row sm:gap-6')}>
