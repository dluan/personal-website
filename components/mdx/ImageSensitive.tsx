...
export default function ImageSensitive({ message = '', ...props }: ImageSensitiveProps) {
  ...
  return (
    <div className={clsx('mdx-image-sensitive relative')}>
      ...
      {!isShown && (
        ...
        >
          <div className={clsx('flex flex-col items-center gap-1 px-4 text-center')}>
            <EyeSlash className={clsx('mb-2 h-5 w-5')} />
            <div className={clsx('font-bold')}>{message || 'Sensitive Content'}</div>
            <p className={clsx('text-sm')}>WARNING: This image is flagged as sensitive content.</p>
          </div>
        </div>
      )}
      <Image immersive={isShown} {...props} alt="" />
    </div>
  )
}
