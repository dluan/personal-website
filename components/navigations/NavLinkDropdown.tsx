...
const LinkRef = forwardRef(({ href, children, ...rest }: LinkRefProps, ref: Ref<HTMLAnchorElement>) => (
        ...
        {({ open }) => (
          <>
            <Menu.Button className={clsx('nav-link nav-link--label ml-2')}>
              {title}
              <ChevronRightIcon className={clsx('h-3 w-3 rotate-90', [open && '-rotate-90'])} />
            </Menu.Button>
            {open && (
              <Menu.Items
                static
                ...
                {items.map((item) => (
                  <Menu.Item key={item.href}>
                    {({ active }) => (
                      <LinkRef
                        href={item.href}
                        className={clsx('nav-link h-8 text-xs font-plusjakartasans', [active && 'nav-link--focus'])}
                      >
                        {item.title}
                      </LinkRef>
                    )}
