...
function PageHeader({ title, description, caption = '', headerImage = null }: PageHeaderProps) {
  return (
    <header
      id="page-header"
      className={clsx(
        'background-grid background-grid--fade-out border-divider-light z-[900] mb-10 border-b pt-32 pb-10 font-plusjakartasans',
        ...
        <div className={clsx('content-wrapper absolute inset-0 overflow-hidden')}>
          ...
          >
            <div className={clsx('content-wrapper relative h-full')}>
              <div className={clsx('absolute right-0 -top-24 bottom-0')}>{headerImage}</div>
            </div>
          </div>
        </div>
      )}
