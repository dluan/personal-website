...
function AppWindow({ children = null, type = 'app', browserTabs = [] }: PropsWithChildren<AppWindowProps>) {
  ...
  return (
    ...
    >
      <div
        className={clsx('border-divider-light relative box-border border-b', 'dark:border-divider-dark', [
      ...
      >
        <div className={clsx('absolute left-4 top-0 flex h-10 items-center gap-1.5')}>
          ...
          <div className={clsx('h-3 w-3 rounded-full bg-amber-300', 'dark:bg-slate-500')} />
          <div className={clsx('h-3 w-3 rounded-full bg-green-300', 'dark:bg-slate-500')} />
        </div>
        {type === 'browser' && (
          <>
            ...
            {isWithBrowserTabs && (
              <div className={clsx('mt-2 flex gap-2 px-3')}>
                {browserTabs.map(({ icon, title, isActive }) => (
                  <BrowserTab key={title} icon={icon} title={title} isActive={isActive} />
                ))}
              </div>
            )}
          </>
